...
import { range } from "../../backend/util";
...
export async function performQuery(sql: string, parameters?: any[]) {
  try {
    ...
    const queryResult = await getPool().query(sql, parameters);
...
export function queryObjectBuilder<T extends QueryResultRow, R = BaseQueryObject<T>>(
  table: string,
  overrideObject?: R
) {
  return Object.assign({
    ...
    async getById(id: number) {
      return performQuery(`SELECT * FROM ${table} WHERE ${table}.id = $1`, [id]);
    },
    async getByExample(object: Partial<T>) {
      const keys = Object.keys(object);
      const parameters = keys.map((key) => (object as any)[key]);
      const args = [...range(1, keys.length)].map((value) => `${table}.${keys[value - 1]} = $${value}`).join(" AND ");
